...
class BulkAction {
  public const ACTION_ID = 'tag_by_upholstery_service';
  public const ACTION_NAME = 'Tag by upholstery service';
  public const ATTACHMENT_FIELD_KEY = 'attachment_upholstery_photos';
  public const COLUMN_ID = 'upholstery_services';
  public const COLUMN_LABEL = 'Upholstery Services';
  public const QUERY_ARG = 'tagged-by-upholstery-service';
  ...
  private static function tagging_by_upholstery_service($action) {
    return preg_match('/Tag\sby\supholstery\sservice/', $action);
  }

  private static function prioritize_column($columns) {
    $column_ids = array_keys($columns);

    $upholstery_services_column_index =
      array_search(self::COLUMN_ID, $column_ids);
    $title_column_index = array_search('title', $column_ids);

    $from_index = $upholstery_services_column_index;
  ...
  public function show_notice() {
    ...
    if (isset($status)) {
      if ($status === 'success') {
        ...
        $message .= "<p>Successfully tagged selected items by upholstery ";
  ...
  public function handle_action($redirect_to, $doaction, $post_ids) {
    ...
    if (self::tagging_by_upholstery_service($doaction)) {
      $status = false;
